...
class LoraInjectedLinear(nn.Module):
    def __init__(
        ...
        # nn.init.normal_(self.lora_mid.weight, std=1 / r)
        nn.init.zeros_(self.lora_mid.weight)
        nn.init.zeros_(self.lora_up.weight)

    def forward(self, input):
        return (
            self.linear(input)
            + self.dropout(self.lora_up(self.selector(self.lora_mid(self.lora_down(input)))))
            * self.scale
        )
...
def convert_to_motion_lora_key(key: str):
    up_or_down = "up" if "_up" in key else "down"
    module_key = key.split(".")[-3]
    
    key = key.replace(
        f".{module_key}.lora_{up_or_down}.weight", 
        f".processor.{module_key}_lora.{up_or_down}.weight"
    )
    return key

def spatial_or_temporal_lora(key: str, use_motion_lora_format: bool = False):
    is_temporal = False
    
    if 'motion' in key or 'temp' in key:
        is_temporal = True
        
        if use_motion_lora_format:
           key = convert_to_motion_lora_key(key)
        
    return key, is_temporal

def replace_key(key: str):
    attns = ["attn1", "attn2"]
    for attn in attns:
        key = key.replace(attn, f"{attn}.processor")
    return key

def convert_unet_state_dict(state_dict: dict):
    process_dict = {replace_key(k): v for k, v in state_dict.items()}
        
    replace_keys = ["to_q", "to_k", "to_v", "to_out", "proj_in", "ff.net"]
    save_dict = {}
    for k, v in process_dict.items():
        if any(["lora_up" in k, "lora_down" in k]) and any([rk in k for rk in replace_keys]):
            up_or_down = "up" if "lora_up" in k else "down"
            k = k.replace(f".lora_{up_or_down}", "")

            for rk in replace_keys:
                if rk in k:
                    if rk == "to_out":
                        k = k.replace("to_out.0", rk)

                    split_key = k.split(".")
                    split_key[-2] = f"{split_key[-2]}_lora.{up_or_down}.weight"
                    del split_key[-1]
...
def save_lora_weight(
    ...
    if use_motion_lora_format:
        assert "Motion LoRA format is only available for safetensors."
...
def save_lora_safetensors(
    ...
    only_save_motion = False
    for replace_module in target_replace_module:
        if any([path in replace_module for path in ["Temporal", "Versatile"]]):
            only_save_motion = True
            break

    for k, v in current_state_dict.items():
        if 'lora' in k and any(['lora_up' in k, 'lora_down' in k]):
            if save_all:
                save_key = k
            else:
                save_key, is_temporal = spatial_or_temporal_lora(k, use_motion_lora_format)
                is_spatial = not is_temporal

                if all([only_save_motion, not is_spatial, use_motion_lora_format]):
                    pass
                else:
                    if is_spatial and only_save_motion:
                        continue
                    
                    if is_temporal and not only_save_motion:
                        continue

            save_dict[save_key] = v.to('cpu', dtype=torch.float32)

            if verbose:
                print(target_replace_module, f"Saving: {save_key}")

    # By default, we convert the Spatial LoRA for use in ComfyUI.
    if not only_save_motion:
        save_dict = convert_unet_state_dict(save_dict)

    safe_save(
        save_dict, 
        path, 
        metadata={"model_type": "motion_director", "rank": str(lora_rank)}
    )
